# CityHash64 install-path hash (Firefox nsXREDirProvider::GetInstallHash).
if (-not ("StealthCityHash64" -as [type])) {
    Add-Type -TypeDefinition @"
using System;
using System.Text;

public static class StealthCityHash64 {
    private const ulong k0 = 0xc3a5c85c97cb3127UL;
    private const ulong k1 = 0xb492b66fbe98f273UL;
    private const ulong k2 = 0x9ae16a3b2f90404fUL;
    private static ulong Fetch64(byte[] s, int pos) { return BitConverter.ToUInt64(s, pos); }
    private static uint Fetch32(byte[] s, int pos) { return BitConverter.ToUInt32(s, pos); }
    private static ulong Rotate(ulong val, int shift) { return shift == 0 ? val : (val >> shift) | (val << (64 - shift)); }
    private static ulong ShiftMix(ulong val) { return val ^ (val >> 47); }
    private static ulong HashLen16(ulong u, ulong v) { return Hash128To64(u, v); }
    ...
    public static string HashPath(string path) {
        ...
        return Hash64(bytes).ToString("X");
    }
}
"@
}

function Get-StealthFirefoxInstallHash {
    param([string]$StealthExe)

    if (-not $StealthExe -or -not (Test-Path -LiteralPath $StealthExe)) {
        return $null
    }

    $engineDir = (Get-Item -LiteralPath $StealthExe).Directory.FullName
    return [StealthCityHash64]::HashPath($engineDir).ToUpperInvariant()
}
